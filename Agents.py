...
class Simple_Agent(Agent): #plays games with 2 actions, using a single parameter
    def __init__(self, env, learning_rate=0.001, n_units_critic = 20, gamma = 0.95, agent_idx = 0, critic_variant = Critic_Variant.INDEPENDENT, p_init = 0.25):
        super().__init__(env, learning_rate, gamma, agent_idx)
        ...
        with tf.variable_scope('Actor', reuse = tf.AUTO_REUSE):
            initial_value = math.log(p_init / (1-p_init))
            self.theta = tf.Variable(tf.random_normal([1],mean = initial_value, stddev=0)) 
    ...
    def __str__(self):
        return 'Agent_'+str(self.agent_idx)
